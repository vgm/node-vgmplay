...
S.prototype.mixStereo = function(buf,len,z) {
	...
	(function(p){
		...
		j = -1; while (++j<len) {
			...
			//if (!buf[n]) buf[n] = 0;
			buf[n++] += vr*_sc*_v;
			//if (isNaN(vl)||isNaN(vr)) throw ("sn::mix NAN"+i);
			//console.log("sn::mix",vl,vr);
			//p.ToneFreqVals[3] |= 0;
		}
		//console.log("***:"+fv.toString(', '));
		//console.log("***:"+nsr.toString(','));
...
module.exports = {
	get label(){return "SN76489";},
	get channels(){return ["Ch 1","Ch 2","Ch 3","Noise"];},
	"create":function(pcl, sr, num){
		var ret = new SN76489();
		ret.init(pcl, sr);
		ret.config(
			ret.ENUM.mute_values.MUTE_ALLON,
			ret.ENUM.boost_modes.BOOST_OFF,
			ret.ENUM.volume_modes.VOL_FULL,
			ret.ENUM.feedback_patterns.FB_SEGAVDP,
			16//settings.psg.shift_width
		);
		return ret;
	}
};
