...
OPN.SetPrescaler = function(x, r) {
	...
	//console.log("EG_TIMER_ADD",x.OPN.eg.timer_add);
...
Y.prototype.init = function(clock,rate) {
	...
	if (this.chip instanceof YMX) this.chip.ST.clock = clock||7670448, this.chip.ST.rate = rate||44100;
	else this.chip = new YMX(clock, rate);
...
Y.prototype.update = function(len) {
	...
	/* buffering */
	var _adv_eg;
	if (cfg.mode) _adv_eg = function(c) {	// vb
		c.OPN.eg.timer += c.OPN.eg.timer_add;	/* advance envelope generator */
		while (c.OPN.eg.timer>=c.OPN.eg.timer_overflow) {
			c.OPN.eg.timer -= c.OPN.eg.timer_overflow;
			++c.OPN.eg.cnt;
			advance_eg_channels(c, c.OPN.eg.cnt);
		}
	};
	else _adv_eg = function(c) {	// gpgx
		++c.OPN.eg.timer;	/* advance envelope generator */
		if (c.OPN.eg.timer>=3) {
			c.OPN.eg.timer = 0;
			++c.OPN.eg.cnt;
			advance_eg_channels(c, c.OPN.eg.cnt);
	...
	i = -1; while (++i<len) {
		...
		advance_lfo(this.chip);	/* advance LFO */
		_adv_eg(this.chip);	/* EG is updated every 3 samples */
		/* buffering */
		buf[0][i] = lt;
		buf[1][i] = rt;
...
Y.prototype.mixStereo = function(buf,len,z) {
	//// update length is given in samples
	//// but needs to calculate in chip cycles,
	//// adjust afterwards +neo
	var num = len*this.ratio;	// num cycles
	if(cfg.debug)
		console.log("==== YM::update","samples="+len,"cycles="+num);
	var j, lt, rt;
	var is_csm = !!(this.chip.OPN.ST.mode&0xc0), dis_csm, CL = this.chip.CH.length;
	//console.log("YM::update","CSM");
	//// TODO: USE Array.forEach INSTEAD SOMEHOW
	var i = -1; while (++i<CL) {
		if (!is_csm) this.chip.CH[i].refresh_fc_eg(this.chip);
		else if (this.chip.CH[i].canCSM) {	/* 3SLOT MODE (operator order is 0,1,3,2) */
			if (this.chip.CH[i].SLOT[_SLOT[0]].Incr===-1) {
	...
	//var msg = [];
	//var z = 1.0*this.chip.OPN.ST.scale, q = (len*z+0.5)|0;//(len*this.chip.OPN.ST.scale+0.5)|0;	// len;
	var didOverflow, didDac;
	var _sc = 1.0/this.attenuation, n = z|0;
	var _v = this.volume/100.0;
	/* buffering */
	var _adv_eg;
	if (cfg.mode) _adv_eg = function(c) {	// vb
		//console.log("YM:update","ADV_EG","VB");
		c.OPN.eg.timer += c.OPN.eg.timer_add;	/* advance envelope generator */
		while (c.OPN.eg.timer>=c.OPN.eg.timer_overflow) {
			c.OPN.eg.timer -= c.OPN.eg.timer_overflow;
			++c.OPN.eg.cnt;
			//console.log("YM:update","ADV_EG",c.OPN.eg.timer,c.OPN.eg.timer_overflow,c.OPN.eg.cnt);
			advance_eg_channels(c, c.OPN.eg.cnt);
		}
	};
	else _adv_eg = function(c) {	// gpgx
	...
	i = -1; while (++i<len) {
		...
		//console.log("YM:update",i,lt,rt);
		/* advance LFO */
		advance_lfo(this.chip);
		//console.log("YM:update","POST ADV_LFO");
		/* EG is updated every 3 samples */
		_adv_eg(this.chip);
		//console.log("YM:update","POST ADV_EG");
		/* buffering */
		buf[n++] += lt*_sc*_v;
		buf[n++] += rt*_sc*_v;
		/* CSM mode: if CSM Key ON has occured, CSM Key OFF need to be sent       */
		/* only if Timer A does not overflow again (i.e CSM Key ON not set again) */
		this.chip.OPN.SL3.key_csm <<= 1;
		//console.log("YM:update","TIMER A");
		INTERNAL_TIMER_A(this.chip);	/* timer A control */
		//console.log("YM:update","CHK CSM DISABLED");
		if (dis_csm) {	/* CSM Mode Key ON still disabled */
			this.chip.OPN.SL3.key_csm = 0;	/* CSM Mode Key OFF (verified by Nemesis on real hardware) */
		}
	}
...
module.exports = {
	get label(){return "YM2612"},
	get channels(){return ["Ch 1","Ch 2","Ch 3","Ch 4","Ch 5","Ch 6"];},
